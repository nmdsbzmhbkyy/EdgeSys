...
func (m *SysTenantModelImpl) FindOne(tenantId int64) *entity.SysTenants {
	...
	err := pkg.Db.Table(m.table).Where("id = ?", tenantId).First(resData).Error
...
func (m *SysTenantModelImpl) Update(data entity.SysTenants) *entity.SysTenants {
	biz.ErrIsNil(pkg.Db.Table(m.table).Updates(&data).Error, "修改SysTenant失败")
	return &data
}

func (m *SysTenantModelImpl) Delete(tenantIds []int64) {
	biz.ErrIsNil(pkg.Db.Table(m.table).Delete(&entity.SysTenants{}, "id in (?)", tenantIds).Error, "删除SysTenant失败")
}
